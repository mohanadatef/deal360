		...
		foreach(language() as $lang)
		{
			if($lang->code == 'en')
			{
				$rules['address.'.$lang->code]=['required','string',Rule::unique('translations','value')
					->where('category_type',Agency::class)->where('key','address')->where('language_id',$lang->id)];
				$rules['about_me.'.$lang->code]=['required','string',Rule::unique('translations','value')
					->where('category_type',Agency::class)->where('key','about_me')->where('language_id',$lang->id)];
			}else
			{
