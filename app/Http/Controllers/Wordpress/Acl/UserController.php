        ...
        public function store($response)
        {
                    ...
                    if(!empty($user['username'])&&!in_array($user['username'],$username)&&!in_array($user['username'],$full_name_users))
                    {
                        if(!empty($user['full_name'])&&!in_array($user['full_name'],$full_name)&&!in_array($user['full_name'],$username_users))
                        {
                            ...
                            $data_user[]=array('id'      =>$count_user+$key+1,'wp_user_id'=>$user['wp_user_id'],
                                               ...
                                               'email'   =>empty($user['mail'])?'mail@test.com'.($count_user+$key+1):$user['mail'],
                                               ...
                                               'website' =>$user['website_url'],'facebook'=>$user['facebook_url'],
                                               'twitter' =>$user['twitter_url'],'instagram'=>$user['instagram_url'],
                                               'approve' =>1,'country_id'=>1,'email_verified_at'=>Carbon::now(),
                            ...
                            if(!empty($user['role_id']))
                            {
                                if($user['role_id']==2)
                                {
                                    $agent[]=array('user_id'    =>$count_user+$key+1,'company_id'=>0,
                                                   'wp_agent_id'=>$user['wp_user_id']);
                                    if(!empty($user['agent']['about_me']))
                                    {
                                        foreach($language as $lang)
                                        {
                                            $agent_translation[]=array('category_type'=>Agent::class,
                                                                       'category_id'  =>$count_user+$key+1,
                                                                       'key'          =>'about_me',
                                                                       'value'        =>$user['agent']['about_me'],
                                                                       'language_id'  =>$lang->id);
                                        }
                                    }
                                }
            ...
            foreach($agency_agent as $agencyagent)
            {
                foreach($agent as $key=>$agentid)
                {
                    ...
                    {
                        $agent[]=array('user_id'    =>$agentid['user_id'],'company_id'=>$agencyagent['agency_id'],
                                       'wp_agent_id'=>$agentid['wp_agent_id']);
                        unset($agent[$key]);
                    }
                }
            }
