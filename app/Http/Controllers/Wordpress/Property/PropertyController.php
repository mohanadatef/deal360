...
class PropertyController extends Controller
{
    ...
    {
        $link = 'http://crm.deal360.ae/backend/api/fillProperty?num=300&page=1';
    ...
    public function store($response)
    {
        ...
        for ($page = 1; $page <= $last; $page++) {
            executionTime();
            $link = 'http://crm.deal360.ae/backend/api/fillProperty?num=300&page=' . $page;
            executionTime();
            $response = Http::get($link)->json();
            executionTime();
            $id=$id+200*$page;
                    ...
                    if (!in_array($property['wp_id'], $wp_properties_id)) {
                        if (!in_array($property['wp_id'], $ids)) {
                            ...
                            if (empty($property['area']) or $property['area'] == "Not Available") {
                                ...
                                $area = isset($area_array[0][0])? $area_array[0][0] : 0 . '.' . $area;
                            }
                            ...
                            $data_property[] = array('id' => $id, 'wp_property_id' => $property['wp_id'],
                                ...
                                'floor_number' => empty($property['floors_num']) ? 0 : ($property['floors_num'] == "Not Available" || $property['floors_num'] =="غير متاح" ? 0 : $property['floors_num']),
                                'garage' => $garage,
                            ...
                            if (!empty($property['all_images'])) {
                                ...
                                foreach ($property['all_images'] as $property_image) {
                                    executionTime();
            ...
            DB::transaction(function () use ($data_property, $data_translation, $image, $photographer, $amenity_property, $floor_plan, $image_plan, $property_image, $property_image_plan, $data_translation1, $data_translation2, $data_translation3, $data_translation4) {
                ...
                if (!empty($image) && count($image)>0) {
                    foreach ($image as $im) {
                        ...
                        executionTime();
                        $property_imagese[] = array('category_type' => Property::class, 'category_id' => $im['id'], 'image' => $name);
                        executionTime();
                    }
                    executionTime();
                    if (!empty($property_imagese)) {
                        DB::table('images')->insert($property_imagese);
                    }
                }
                executionTime();
