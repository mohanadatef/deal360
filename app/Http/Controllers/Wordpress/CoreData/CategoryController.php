        ...
        foreach ($response['data']['property_category'] as $key => $category) {
            ...
            if (!in_array($category['id'], $wp_categories_id)) {
                ...
                foreach ($language as $lang) {
                    $data_translation[] = array('category_type' => Category::class,
                        'category_id' => $count_category + $key + 1, 'key' => 'title', 'value' => !empty($category['name']) ? $category['name'] : 'free ' . ($key + 1),
                        'language_id' => $lang->id);
                }
            }
        }
         DB::transaction(function () use ($data_category,$data_translation) {
