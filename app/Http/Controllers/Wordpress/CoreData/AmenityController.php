        ...
        foreach ($response['data']['property_features'] as $key => $amenity) {
            ...
            if (!in_array($amenity['id'], $wp_amenities_id)) {
                ...
                foreach ($language as $lang) {
                    $data_translation[] = array('category_type' => Amenity::class,
                        'category_id' => $count_amenity + $key + 1, 'key' => 'title', 'value' => !empty($amenity['name']) ? $amenity['name'] : 'free ' . ($key + 1),
                        'language_id' => $lang->id);
                }
            }
        }
         DB::transaction(function () use ($data_amenity,$data_translation) {
            DB::table('amenities')->insert($data_amenity);
